...
def generar_numero_cotizacion(db: Session, año: int) -> str:
    ...
    ultima_cotizacion = db.query(models.Cotizacion).filter(
        extract('year', models.Cotizacion.fecha_cotizacion) == año
    ).order_by(models.Cotizacion.id_cotizacion.desc()).first()
...
def crear_cotizacion(
    ...
    cliente = db.query(models.Cliente).filter(models.Cliente.id_cliente == cotizacion.id_cliente).first()
    if not cliente:
...
def listar_cotizaciones(
    ...
    cotizaciones = query.order_by(models.Cotizacion.fecha_cotizacion.desc()).offset(skip).limit(limit).all()
    return cotizaciones
...
def convertir_cotizacion_a_venta(
    ...
    if estado_actual not in ("Pendiente", "Aceptada"):
        raise HTTPException(
            status_code=400,
            detail="Solo se pueden convertir cotizaciones pendientes o aceptadas"
        )
