...
from app.models.pagination import PaginationParams, paginate_query, create_paginated_response
...
def search_productos_advanced(
    ...
    query = query.order_by(
        Producto.nombre_producto.ilike(search_filter).desc(),
...
def get_productos_stats(db: Session, filters: Optional[ProductoFilters] = None) -> Dict[str, Any]:
    ...
    precio_promedio = db.query(
        func.avg(Producto.precio_compra)
    ).filter(
        Producto.estado == "Activo"
    ).scalar() or 0.0
    ...
    return {
        ...
        "porcentaje_bajo_stock": round(
            (productos_bajo_stock / productos_activos * 100) if productos_activos > 0 else 0,
            2
        )
    }
...
def get_productos_por_laboratorio_stats(db: Session) -> List[Dict[str, Any]]:
    ...
    results = db.query(
        ...
        Producto, Producto.id_laboratorio == Laboratorio.id_laboratorio
    ).filter(
        Producto.estado == "Activo"
    ).group_by(
        Laboratorio.nombre_laboratorio
    ).order_by(
        func.count(Producto.id_producto).desc()
    ).all()
...
def get_productos_por_seccion_stats(db: Session) -> List[Dict[str, Any]]:
    ...
    results = db.query(
        ...
        Producto, Producto.id_seccion == Seccion.id_seccion
    ).filter(
        Producto.estado == "Activo"
    ).group_by(
        Seccion.nombre_seccion
